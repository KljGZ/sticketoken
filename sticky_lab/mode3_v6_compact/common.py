"""Shared Compact configuration and artifact helpers."""
...
def load_config(path: Path) -> dict[str, Any]:
    ...
    if config.get("protocol_version") != 6 or config.get("compact_protocol_version") != 1:
        raise RuntimeError("not a V6 Compact configuration")
    if config.get("scope", {}).get("only_mode") != 3:
        raise RuntimeError("V6 Compact scope must be Mode 3 only")
    return config
