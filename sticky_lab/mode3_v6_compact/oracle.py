...
class CompactFinalOracle:
    ...
    def encode(self, texts: Sequence[str], *, metadata: Mapping[str, Any] | None = None) -> np.ndarray:
        ...
        return normalize_rows(np.asarray(vectors, dtype=np.float64)).astype(np.float32)
...
def write_embedding_cache(
    ...
    *,
    ...
    manifest = {
        "schema_version": "mode3-v6-compact-embedding-cache-v1",
        "role": role,
...
def load_embedding_cache(
    ...
    *,
    ...
    if expected_role is not None and manifest["role"] != expected_role:
        raise RuntimeError("embedding cache role mismatch")
    if expected_records_hash is not None and manifest["records_sha256"] != expected_records_hash:
        raise RuntimeError("embedding cache record mismatch")
    ...
    if list(vectors.shape) != manifest["shape"] or str(vectors.dtype) != manifest["dtype"]:
        raise RuntimeError("embedding cache shape/dtype mismatch")
    return vectors
