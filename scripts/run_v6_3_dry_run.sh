...
export V6_3_OUTPUT="${V6_3_DRY_OUTPUT:-/mnt/data/jkl/StickyToken-v6-3-dry-run-r4/sticky_lab/sentence_t5_base/mode3_v6_3_light}"
export V6_3_PROFILE=dry_run
export V6_3_GPUS="${V6_3_DRY_GPUS:-4}"

IFS=',' read -r -a dry_gpu_array <<<"$V6_3_GPUS"
[[ "${#dry_gpu_array[@]}" -eq 1 ]] || {
  echo "V6.3 dry-run requires exactly one physical GPU: $V6_3_GPUS" >&2
  exit 64
}
...
if len(gpus) != 1 or gpus[0] not in range(4, 8): raise SystemExit(f'invalid dry-run GPU binding: {gpus}')
...
value={"schema_version":"mode3-v6-3-dry-run-v1","status":"DRY_RUN_PASSED","legal_tokens":64,"one_physical_gpu":gpus[0],"authorized_physical_gpus":gpus,"physical_gpus_0_3_used":False,"marker_boundary_replay_passed":True,"scientific_claims_allowed":False,"utc":time.strftime('%Y-%m-%dT%H:%M:%SZ',time.gmtime())}
