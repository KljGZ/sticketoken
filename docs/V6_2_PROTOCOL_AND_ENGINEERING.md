...
The corpus must contain at least four genuine IID sources plus four source-isolated OOD datasets. `scripts/build_v6_2_corpus.py` preserves the five V6 files byte-for-byte and adds the remote machine's existing MS MARCO, HotpotQA, and Natural Questions raw snapshots. Every complete raw JSONL file is bound by SHA-256 in the manifest. It never manufactures source diversity by relabeling one dataset.
