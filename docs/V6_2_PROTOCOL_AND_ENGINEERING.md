...
The corpus must contain at least four genuine IID sources plus four source-isolated OOD datasets. `scripts/build_v6_2_corpus.py` preserves the five V6 files byte-for-byte and adds pinned Wikipedia, SQuAD-context, and arXiv-abstract sources. It never manufactures source diversity by relabeling one dataset.
